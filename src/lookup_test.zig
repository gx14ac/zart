...
test "IdxToFringeRoutes" {
    ...
    for (test_cases) |tc| {
        // Go BART: got := IdxToFringeRoutes(tc.idx).All()
        const bitset_ptr = lookup_fringe.idxToFringeRoutes(tc.idx);
        const got = try bitset_ptr.all(std.testing.allocator);
...
test "IdxToPrefixRoutes" {
    ...
    for (test_cases) |tc| {
        // Go BART: got := IdxToPrefixRoutes(tc.idx).All()
        const bitset_ptr = lookup_prefix.idxToPrefixRoutes(tc.idx);
        const got = try bitset_ptr.all(std.testing.allocator);
        defer std.testing.allocator.free(got);
